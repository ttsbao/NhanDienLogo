...
def train(hyp,  # path/to/hyp.yaml or hyp dictionary
    ...
    if RANK in [-1, 0]:
        ...
        wandb_logger = WandbLogger(opt, save_dir.stem, run_id, data_dict, socketname=nameclient)
    ...
    if pretrained:
        ...
        model = Model(cfg or ckpt['model'].yaml, ch=3, nc=nc, anchors=hyp.get('anchors'), socketname=nameclient).to(device)  # create
        ...
        model = Model(cfg, ch=3, nc=nc, anchors=hyp.get('anchors'), socketname=nameclient).to(device)  # create
    with torch_distributed_zero_first(RANK):
    ...
    imgsz, imgsz_test = [check_img_size(x, gs, socketname=nameclient) for x in opt.img_size]  # verify imgsz are gs-multiples
    ...
    dataloader, dataset = create_dataloader(train_path, imgsz, batch_size // WORLD_SIZE, gs, single_cls,
                                            ...
                                            workers=workers,
                                            image_weights=opt.image_weights, quad=opt.quad, prefix=colorstr('train: '), socketname=nameclient)
    ...
    if RANK in [-1, 0]:
        testloader = create_dataloader(test_path, imgsz_test, batch_size // WORLD_SIZE * 2, gs, single_cls,
                                       ...
                                       workers=workers,
                                       pad=0.5, prefix=colorstr('val: '), socketname=nameclient)[0]
        ...
        if not resume:
            ...
            if not opt.noautoanchor:
                check_anchors(dataset, model=model, thr=hyp['anchor_t'], imgsz=imgsz, socketname=nameclient)
    ...
    socket.emit_server('Start Training', type='Start Training|||' + str(epochs))
    num = 0
    for epoch in range(start_epoch, epochs):  # epoch ------------------------------------------------------------------
        num += 1
        socket.emit_server(('%10s' * 8) % ('Epoch', 'gpu_mem', 'box', 'obj', 'cls', 'total', 'labels', 'img_size'), type='Training|||'+str(num/epochs)+'|||'+str(num))
        model.train()
        ...
        if RANK in [-1, 0]:
            ...
            if not notest or final_epoch:  # Calculate mAP
                ...
                results, maps, _ = test.run(data_dict,
                                            ...
                                            socketname=socket.client_key)

            # Write
            with open(results_file, 'a') as f:
                f.write(s + '%10.4g' * 7 % results + '\n')  # append metrics, val_loss
                #socket.emit_server('%10.4g' * 7 % results + '\n')
    ...
    socket.emit_server("Train Completed", type='Training|||OK')
    return results
...
def main(opt):
    ...
    if RANK in [-1, 0]:
        ...
        #socket.emit_server(colorstr('train: ') + ', '.join(f'{k}={v}' for k, v in vars(opt).items()))
        check_git_status()
...
if __name__ == "__main__":
    opt = parse_opt()
    nameclient = socket.get_name(opt.data)
    socket.set_client_key(nameclient)
    socket.emit_server("Starting Training ... ")
    socket.emit_server("Config Model ...")
    main(opt)
