...
def check_img_size(img_size, s=32, socketname=None):
    socket.set_client_key(socketname)
    # Verify img_size is a multiple of stride s
    new_size = make_divisible(img_size, int(s))  # ceil gs-multiple
    if new_size != img_size:
        print('WARNING: --img-size %g must be multiple of max stride %g, updating to %g' % (img_size, s, new_size))
        #socket.emit_server('WARNING: --img-size %g must be multiple of max stride %g, updating to %g' % (img_size, s, new_size))
    return new_size
...
def strip_optimizer(f='best.pt', s='', socketname=None):  # from utils.general import *; strip_optimizer()
    ...
    socket.emit_server(f"Optimizer stripped from {f},{(' saved as %s,' % s) if s else ''} {mb:.1f}MB")
